...
impl UnixStreamExt for UnixStream {
    ...
    unsafe fn readv_overlapped(&self,
                              ...
                              overlapped: *mut OVERLAPPED)
                              -> io::Result<Option<usize>> {
        let mut wsabufs: Vec<WSABUF> = Vec::with_capacity(bufs.len());
        for buf in bufs.iter() {
            let wsabuf = slice2buf(buf);
            wsabufs.push(wsabuf);
        };
        let mut flags = 0;
        let mut bytes_read: DWORD = 0;
        let r = WSARecv(self.as_raw_socket() as SOCKET,
                        wsabufs.as_mut_ptr(), wsabufs.len() as _,
                        &mut bytes_read, &mut flags, overlapped, None);
        cvt(r, bytes_read)
    }
